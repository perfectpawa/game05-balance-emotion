...
public class GridSystem2D<T> {
    ...
    public static GridSystem2D<T> VerticalGrid(int width, int height, int extra, float cellSize, Vector3 origin, bool debug = false) {
        return new GridSystem2D<T>(width, height + extra, cellSize, origin, new VerticalConverter(), debug);
    }
    ...
    TextMeshPro CreateWorldText(GameObject parent, string text, Vector3 position, Vector3 dir, 
        int fontSize = 4, Color color = default, TextAlignmentOptions textAnchor = TextAlignmentOptions.Center, int sortingOrder = 0) 
    {
        ...
        textMeshPro.color = color == default ? Color.black : color;
